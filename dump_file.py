...
p.add_argument('--raw-order', action='store_true', help='Provide text in raw (content stream) order')
p.add_argument('--fixed-pitch', type=float, default=0.0, help='Fixed pitch - param for text analysis - app. max space size')
args=p.parse_args()
d=pdf.Document(args.document, args.raw_order, args.phys_layout, args.fixed_pitch)  # @UndefinedVariable
